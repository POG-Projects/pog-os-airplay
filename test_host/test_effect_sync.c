...
int main(void) {
  ...
  assert(!effect_sync_join(&sync, group, "follower", leader, 0, 0, "unknown"));
  assert(
      effect_sync_join(&sync, group, "follower", leader, 0, 0, "bass_pulse"));
  ...
  effect_sync_frame_t utc = {3, 1080, 1760000000100ULL, 0, .5f, .5f, .5f, 200};
  ...
  assert(fabsf(effect_sync_pixel_position(299, 300) - (299.5f / 300.0f)) <
         .001f);
  ...
  effect_sync_pixel_t pixel =
      effect_sync_visualizer_pixel(EFFECT_VISUALIZER_SPECTRUM, .5f, &out);
  assert(fabsf(pixel.hue - (1.0f / 3.0f)) < .001f);
  assert(fabsf(pixel.value - .5f) < .001f);
  assert(effect_sync_visualizer_pixel(EFFECT_VISUALIZER_VU_METER, .25f, &out)
             .value == 1.0f);
  assert(effect_sync_visualizer_pixel(EFFECT_VISUALIZER_VU_METER, .75f, &out)
             .value == 0.0f);
  effect_sync_pixel_t left =
      effect_sync_visualizer_pixel(EFFECT_VISUALIZER_BASS_PULSE, .1f, &out);
  effect_sync_pixel_t right =
      effect_sync_visualizer_pixel(EFFECT_VISUALIZER_BASS_PULSE, .9f, &out);
