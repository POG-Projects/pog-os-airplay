...
int pogvoice_resampler_init(pogvoice_resampler_t *r, unsigned in, unsigned out) {
  if (!r || in < 8000 || in > 48000 || out < 8000 || out > 48000) return -1;
  memset(r, 0, sizeof(*r));
  r->input_rate = in;
  r->output_rate = out;
  ...
  for (unsigned p = 0; p <= POGVOICE_FIR_PHASES; ++p) {
    ...
    for (unsigned k = 0; k < POGVOICE_FIR_TAPS; ++k) {
      double x = k - (POGVOICE_FIR_TAPS - 1) / 2.0 +
                 (double)p / POGVOICE_FIR_PHASES;
      double a = 2 * pi * k / (POGVOICE_FIR_TAPS - 1);
      double window = .35875 - .48829 * cos(a) + .14128 * cos(2 * a) -
                      .01168 * cos(3 * a);
      coefficients[k] = (fabs(x) < 1e-9 ? cutoff
                                         : sin(pi * cutoff * x) / (pi * x)) * window;
      sum += coefficients[k];
    }
    int total = 0;
    for (unsigned k = 0; k < POGVOICE_FIR_TAPS; ++k) {
...
int pogvoice_resampler_process(pogvoice_resampler_t *r, const int16_t *in,
  ...
  if (!r || !r->input_rate || !r->output_rate || !in || !out ||
      count > 160 || cap > 1024) return -1;
  unsigned span = (unsigned)count * r->output_rate;
  size_t required = span > r->phase
      ? (span - r->phase + r->input_rate - 1) / r->input_rate : 0;
  if (cap < required) return -1; /* Reject before changing stream state. */
  size_t used = 0;
