...
bool pogvoice_feedback_render(uint8_t *rgb, size_t count,
  ...
  switch (state) {
  case POGVOICE_LIGHT_CONNECTING: r = 140; g = 80; b = 255; break;
  case POGVOICE_LIGHT_LISTENING: g = 180; b = 255; break;
  case POGVOICE_LIGHT_THINKING: r = 255; g = 150; b = 20; break;
  case POGVOICE_LIGHT_SPEAKING: r = 35; g = 255; b = 110; break;
  case POGVOICE_LIGHT_ERROR: r = 255; g = 25; b = 15; break;
  default: return false;
  }
  for (size_t i = 0; i < count; i++) {
    unsigned value = intensity;
    if (state == POGVOICE_LIGHT_LISTENING || state == POGVOICE_LIGHT_SPEAKING) {
      size_t distance = (2 * i + 1 > count) ? 2 * i + 1 - count
                                           : count - (2 * i + 1);
