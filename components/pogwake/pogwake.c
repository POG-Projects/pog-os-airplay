...
void pogwake_init(esp_err_t (*c)(pogmic_stream_cb, void *), bool (*a)(void),
                  esp_err_t (*t)(const char *)) {
  (void)c;
  (void)a;
  (void)t;
}
...
void pogwake_get_status(pogwake_status_t *s) {
  memset(s, 0, sizeof(*s));
}
#else
...
static void monitor(void *arg) {
  ...
  portEXIT_CRITICAL(&s_lock);
  wake_capture_t *c =
      heap_caps_calloc(1, sizeof(*c), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  pogvoice_resampler_t *resampler = heap_caps_calloc(
      1, sizeof(*resampler), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  ...
  c->storage =
      heap_caps_malloc(WAKE_QUEUE_BYTES, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!c->storage)
    goto finish;
  c->queue =
      xStreamBufferCreateStatic(WAKE_QUEUE_BYTES, 2, c->storage, &c->control);
  engine = pogwake_engine_create(model);
  if (!c->queue || !engine ||
      pogvoice_resampler_init(resampler, CONFIG_OUTPUT_SAMPLE_RATE_HZ, 16000))
    goto finish;
  ...
  portEXIT_CRITICAL(&s_lock);
  ESP_LOGI(
      "pogwake", "Local wake listening: %s internal free=%u largest=%u",
  ...
  while (current(generation) && !atomic_load(&c->done)) {
    ...
    size_t bytes =
        xStreamBufferReceive(c->queue, pcm, sizeof(pcm), pdMS_TO_TICKS(50));
    if (!bytes)
      continue;
...
finish:
  if (started) {
    ...
    if (!hit && current(generation) && error == ESP_OK)
      error = atomic_load(&c->error);
  }
  ...
  portEXIT_CRITICAL(&s_lock);
  if (hit)
    ESP_LOGI("pogwake", "Wake detected: %s", pogwake_engine_phrase(model));
  else if (error != ESP_OK && current(generation))
    ESP_LOGW("pogwake", "Wake stopped: %s", esp_err_to_name(error));
  /* Return to the reusable audio worker. */
}

static void tick(void *arg) {
  (void)arg;
  if (xSemaphoreTake(s_control, 0) != pdTRUE)
    return;
  if (!s_available()) {
    portENTER_CRITICAL(&s_lock);
    if (s_status.active)
  ...
  portENTER_CRITICAL(&s_lock);
  bool go = s_status.enabled && !s_suspended && !s_status.active &&
            !mic.active && mic.clock_ready &&
  ...
  if (go) {
    ...
    if (!hit) {
      ...
      s_worker_generation = s_generation;
    }
  }
  portEXIT_CRITICAL(&s_lock);
  if (!go) {
  ...
  if (hit)
    err = s_trigger(pogwake_engine_phrase(model));
  else
    err = pogvoice_worker_submit(monitor, (void *)(uintptr_t)model, 4);
  if (err != ESP_OK) {
    portENTER_CRITICAL(&s_lock);
    s_status.active = false;
    s_status.error = err;
...
void pogwake_init(esp_err_t (*capture)(pogmic_stream_cb, void *),
  ...
  if (nvs_open("pogwake", NVS_READONLY, &n) == ESP_OK) {
    nvs_get_u8(n, "enabled", &enabled);
    nvs_get_u8(n, "model", &model);
    nvs_close(n);
  }
...
esp_err_t pogwake_configure(bool enabled, unsigned model) {
  if (!pogwake_engine_phrase(model))
    return ESP_ERR_INVALID_ARG;
  if (!s_control)
...
void pogwake_get_status(pogwake_status_t *status) {
  ...
  *status = s_status;
  portEXIT_CRITICAL(&s_lock);
  strlcpy(status->phrase, pogwake_engine_phrase(status->model),
          sizeof(status->phrase));
}
#endif
