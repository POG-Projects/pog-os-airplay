...
bool effect_sync_leave(effect_sync_t *state, const char *group_id) {
  if (state == NULL || !state->joined ||
      ...
      strcmp(state->group_id, group_id) != 0)
    return false;
  memset(state, 0, sizeof(*state));
  return true;
}
...
bool effect_sync_push(effect_sync_t *state, const effect_sync_frame_t *frame) {
  if (state == NULL || frame == NULL || !state->joined ||
      ...
      frame->treble > 1)
    return false;
  if (state->has_rendered && !seq_after(frame->seq, state->last_rendered_seq))
    return false;
  size_t at = 0;
  while (at < state->frame_count &&
         seq_after(frame->seq, state->frames[at].seq))
    ++at;
...
#define FRAME_TIME(frame)               \
  ...
  if (target < FRAME_TIME(state->frames[0]))
    return false;
  size_t before = 0;
  while (before + 1 < state->frame_count &&
         FRAME_TIME(state->frames[before + 1]) <= target)
    ++before;
