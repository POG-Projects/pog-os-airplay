...
/* Scan WITHOUT tearing down the STA link. Unlike wifi_scan(), this does not
   call esp_wifi_disconnect()/clear the BSSID, so an active connection
   survives (the radio briefly hops channels during the blocking scan, then
   resumes on the home channel). Used by the web UI so scanning doesn't drop
   AirPlay or take the device offline for seconds. */
esp_err_t wifi_scan_keep_connected(wifi_ap_record_t **ap_list,
  ...
  esp_err_t err = esp_wifi_scan_start(&scan_config, true);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "WiFi scan (keep-connected) failed: %s",
             esp_err_to_name(err));
    return err;
  }
