...
//   0 = text (track info), 1 = artwork (cover art images), 2 = progress.
...
void mdns_airplay_init(void) {
  ...
  // Build a sanitized mDNS hostname. Hostnames cannot contain spaces (a name
  // like "Salon 2" would produce an invalid host "Salon\032 2.local" and make
  // AirPlay clients fail with error 9039). Keep [A-Za-z0-9-] as-is, map space
  // and '_' to '-', drop anything else; strip leading/trailing '-'; fall back
  // to "airplay" if empty; cap at 32 chars. The service INSTANCE names below
  // still use the original device_name (with spaces) so the friendly name
  // keeps showing in the AirPlay menu.
  char host[33];
  size_t hn = 0;
  for (const char *p = device_name; *p && hn < sizeof(host) - 1; p++) {
    char c = *p;
    if ((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') ||
        (c >= '0' && c <= '9') || c == '-') {
      host[hn++] = c;
  ...
  if (lead > 0) {
    memmove(host, host + lead, strlen(host + lead) + 1);
  }
  if (host[0] == '\0') {
    strncpy(host, "airplay", sizeof(host) - 1);
